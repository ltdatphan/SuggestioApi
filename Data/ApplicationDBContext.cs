...
public class ApplicationDBContext : IdentityDbContext<User>
{
    ...
    private void UpdateTimestamps()
    {
        var entities = ChangeTracker.Entries()
            .Where(e => (e.Entity is CuratedList || e.Entity is Item) &&
        ...
        foreach (var entity in entities)
        {
            ...
            if (currEntity is CuratedList)
            {
                ...
                if (entity.State == EntityState.Added) ((CuratedList)currEntity).CreatedAt = currTime;
            }
            else if (currEntity is Item)
            {
                //Cast to Item
                ((Item)currEntity).UpdatedAt = currTime;
                if (entity.State == EntityState.Added) ((Item)currEntity).CreatedAt = currTime;
            }
        }
    }
}